...
public class CredentialService {
  ...
  @Transactional(readOnly = true)
  public List<CredentialSummary> findAllBasedOnSearch(Long userId, String search){
    return credentialRepository.findBySearch(search, userId);
  }

  @Transactional(readOnly = true)
  public CredentialDetail findById(Long id, Long userId) throws NotFoundException, DecryptionFailedException {
    return credentialRepository.findById(id, userId).map(
      credentialDetail -> {
        String decryptedPassword = null;
        try {
          decryptedPassword = encryptionService.decrypt(credentialDetail.password());
        } catch (Exception e) {
    ...
    ).orElseThrow(() -> new NotFoundException("Credential wasn't found"));
  }

  @Transactional
  public Optional<Long> create(CredentialCreate create) throws EncryptionFailedException {
  ...
  @Transactional
  public void updateInformation(Long id, Long userId, CredentialUpdateInformation credentialUpdateInformation){
    credentialRepository.updateInformation(id, userId ,credentialUpdateInformation);
  }

  @Transactional
  public void updatePassword(Long id, Long userId, CredentialUpdatePassword information) throws  EncryptionFailedException {
    String encryptedPassword = null;
    try {
      encryptedPassword = encryptionService.encrypt(information.password());
    } catch (Exception e) {
      throw new EncryptionFailedException(e.getMessage());
    }
    credentialRepository.updatePassword(id, userId, new CredentialUpdatePassword(encryptedPassword));
  }
