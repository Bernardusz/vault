...
public class UserController {
  ...
  @GetMapping("/detail")
  public ResponseEntity<UserPublicInfo> findDetail(@AuthenticationPrincipal User currentUser) {
    return ResponseEntity.ok(userService.findDetail(currentUser.id()));
  }

  @PutMapping("/update/information")
  public ResponseEntity<Void> updateInformation(@AuthenticationPrincipal User currentUser, @RequestBody UserInformationUpdate userData) {
    userService.updateUserInformation(currentUser.id(), userData);
    return ResponseEntity.ok().build();
  }

  @PutMapping("/update/password")
  public ResponseEntity<Void> updatePassword(@AuthenticationPrincipal User currentUser, @RequestBody UserPasswordUpdate userData) {
